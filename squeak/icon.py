...
def render_icon(size: int) -> QImage:
    ...
    Refined "S" monogram: solid pink rounded square, larger white "S",
    very subtle top highlight, plus a small white dot in the corner
    that echoes the "." of the "Squeak." wordmark.
    """
    ...
    # Rounded square background — solid pink, no gradient.
    bg_path = QPainterPath()
    bg_path.addRoundedRect(rect, radius, radius)
    p.fillPath(bg_path, QColor("#EC4899"))

    # Very subtle top highlight for depth (clipped to the rounded shape).
    p.setClipPath(bg_path)
    highlight = QLinearGradient(0, 0, 0, size * 0.55)
    highlight.setColorAt(0.0, QColor(255, 255, 255, 24))
    highlight.setColorAt(1.0, QColor(255, 255, 255, 0))
    p.fillRect(rect, QBrush(highlight))

    # Centered "S" — Black weight, slightly larger than before, tighter spacing.
    font = QFont()
    ...
    # The "." accent — small white dot in the bottom-right, hidden at very small sizes.
    if size >= 64:
        dot_d = max(2.0, size * 0.07)
        margin = size * 0.13
        dot_x = size - margin - dot_d / 2
        dot_y = size - margin - dot_d / 2
        p.setBrush(QColor(255, 255, 255, 240))
        p.setPen(Qt.NoPen)
        p.drawEllipse(int(dot_x - dot_d / 2), int(dot_y - dot_d / 2), int(dot_d), int(dot_d))
